...
function ReviewDialog({ request, onClose }: { request: FundingRequest | null; onClose: () => void }) {
  ...
  return <Dialog open={!!request} onOpenChange={(open) => !open && onClose()}><DialogContent>
    ...
    {request && <div className="rounded-lg bg-surface-muted p-3 text-sm"><p className="font-medium">Requested by {request.requestedByName}</p><p className="mt-2 whitespace-pre-wrap text-foreground-muted">{request.purpose}</p><p className="mt-3 font-semibold"><MoneyDisplay minor={request.amountMinor} currency={request.currency} /></p></div>}
    <form onSubmit={submit} className="space-y-4">
      ...
      <DialogFooter><Button type="button" variant="ghost" onClick={onClose}>Cancel</Button><Button type="submit" variant={decision === "rejected" ? "destructive" : "primary"} loading={review.isPending}>{decision === "approved" ? "Approve funds" : "Reject request"}</Button></DialogFooter>
    </form>
  </DialogContent></Dialog>;
}
...
export default function BudgetsPage() {
  ...
  const rows = useMemo(() => {
    ...
    for (const row of allocations) {
      ...
      else map.set(key, { departmentId: row.departmentId, departmentName: row.departmentName, period: row.period, currency: row.currency, allocatedMinor: row.allocatedMinor, approvedRequestsMinor: 0, approvedExpensesMinor: 0, availableMinor: row.allocatedMinor, pendingRequests: 0 });
    }
    if (month !== "all" && can.can("budget:manage")) {
      for (const department of departments) {
        ...
        if (!map.has(key)) map.set(key, { departmentId: department.id, departmentName: department.name, period: month, currency: baseCurrency, allocatedMinor: 0, approvedRequestsMinor: 0, approvedExpensesMinor: 0, availableMinor: 0, pendingRequests: 0 });
      }
    }
  ...
  const currencyTotals = useMemo(() => {
    const grouped = new Map<string, { allocation: number; additions: number; expenses: number; available: number }>();
    for (const row of summary) {
      const sum = grouped.get(row.currency) ?? { allocation: 0, additions: 0, expenses: 0, available: 0 };
      sum.allocation += row.allocatedMinor; sum.additions += row.approvedRequestsMinor; sum.expenses += row.approvedExpensesMinor; sum.available += row.availableMinor;
