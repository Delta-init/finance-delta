...
import type { CreateVendorCreditInput } from "@delta/shared";
...
export default function NewVendorCreditPage() {
  ...
  async function onSubmit(values: FormValues) {
    const input: CreateVendorCreditInput = {
      ...
      sourceBillId: values.sourceBillId || undefined,
    ...
    try {
      const credit = await createCredit.mutateAsync(input);
      toast.success("Vendor credit created");
      router.push(`/vendor-credits/${credit.id}`);
    } catch (e) {
      toast.error(e instanceof ApiError ? e.message : "Failed to create vendor credit");
    }
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="rounded-lg border border-border bg-surface p-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5 sm:col-span-2">
              <Label>Reason *</Label>
              <Input {...register("reason")} placeholder="e.g. Returned goods, overcharge correction…" />
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" loading={isSubmitting}>Create vendor credit</Button>
        </div>
      </form>
    </div>
  );
