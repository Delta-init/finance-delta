...
import { formatMoney, displayTaxSplit } from "../money/index";
import {
  formatOrgAddress,
  taxNumberLabel,
  type OrganizationSettings,
} from "../schemas/organization.schema";
import { getPrintLabels } from "../i18n/print-labels";
 ...
 *
 * Lives in shared because it is drawn twice: the browser hands it to somebody
 * as a download, and the server attaches it to the email that carries the
 * invoice out. Those used to be one drawing in the browser and no drawing at
 * all on the server, which is why an emailed invoice was a paragraph of HTML
 * with the figures retyped into it.
 *
...
export function buildInvoicePdf(opts: {
  ...
  // Returned rather than saved: what to do with it is the caller's business,
  // and there are two callers who want different things.
  return doc;
}

/** The file name both callers use, so the download and the attachment agree. */
export function invoicePdfName(invoiceNumber: string): string {
  return `${invoiceNumber}.pdf`;
}

/** For the server: the bytes, to attach to a message. */
export function invoicePdfBuffer(opts: Parameters<typeof buildInvoicePdf>[0]): Buffer {
  return Buffer.from(buildInvoicePdf(opts).output("arraybuffer"));
}
