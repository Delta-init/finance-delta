...
#
#   ./scripts/crm-handover-e2e.sh
#   E2E_API_PORT=4123 ./scripts/crm-handover-e2e.sh
#
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MONGO_PORT="${E2E_MONGO_PORT:-27078}"
API_PORT="${E2E_API_PORT:-4111}"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/crm-handover-e2e.XXXXXX")"
