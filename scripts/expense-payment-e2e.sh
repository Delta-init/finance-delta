#!/usr/bin/env bash
#
# End-to-end test of an expense's payment status.
#
# Stands up a throwaway mongod and a throwaway finance API, then walks an
# expense through being unpaid, falling overdue, being settled and being
# un-settled — over real HTTP, checking the derived status at each step and
# that the list filter agrees with the badge. Tears both down afterwards.
#
...
#
#   ./scripts/expense-payment-e2e.sh
#   E2E_API_PORT=4123 ./scripts/expense-payment-e2e.sh
#
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MONGO_PORT="${E2E_MONGO_PORT:-27082}"
API_PORT="${E2E_API_PORT:-4114}"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/expense-payment-e2e.XXXXXX")"
...
echo "Paying expenses"
if ! bun src/scripts/expense-payment-e2e.ts; then
  echo
