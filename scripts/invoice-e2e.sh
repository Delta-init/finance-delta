#!/usr/bin/env bash
#
# End-to-end test of correcting a sent invoice, and of un-voiding one.
#
# Stands up a throwaway mongod and a throwaway API, then corrects invoices that
# have already gone out and brings voided ones back — checking the floor that
# stops one being edited below what has been received, the status recount that
# follows the new total, and that a restored invoice can be corrected and sent
# again. Tears both down afterwards.
#
...
#
#   ./scripts/invoice-e2e.sh
#   E2E_API_PORT=4123 ./scripts/invoice-e2e.sh
#
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MONGO_PORT="${E2E_MONGO_PORT:-27086}"
API_PORT="${E2E_API_PORT:-4116}"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/invoice-e2e.XXXXXX")"
...
echo "Editing and restoring invoices"
if ! bun src/scripts/invoice-e2e.ts; then
  echo
