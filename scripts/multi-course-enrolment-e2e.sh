#!/usr/bin/env bash
#
# End-to-end test of a lead closing in the CRM and becoming an invoice here.
#
# Stands up a throwaway mongod and a throwaway finance API, then drives the
# whole path over real HTTP: who a sale can be attributed to, whether the
# salesperson can be found, and the signed handover itself. Tears both down
# afterwards.
#
...
#
#   ./scripts/multi-course-enrolment-e2e.sh
#   E2E_API_PORT=4123 ./scripts/multi-course-enrolment-e2e.sh
#
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MONGO_PORT="${E2E_MONGO_PORT:-27091}"
API_PORT="${E2E_API_PORT:-4131}"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/multi-course-enrolment-e2e.XXXXXX")"
...
DB="mongodb://127.0.0.1:$MONGO_PORT/finance-multi-course-e2e"
...
if ! bun src/scripts/multi-course-enrolment-e2e.ts; then
  echo
