#!/usr/bin/env bash
#
# End-to-end test of an accepted quotation becoming an invoice.
#
# Stands up a throwaway mongod and a throwaway finance API, then converts a
# quotation over real HTTP in every mode the endpoint offers — whole, fixed
# amount, percentage and per-line — and checks the guards still refuse what
# they should. Tears both down afterwards.
#
...
#
#   ./scripts/quotation-convert-e2e.sh
#   E2E_API_PORT=4123 ./scripts/quotation-convert-e2e.sh
#
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MONGO_PORT="${E2E_MONGO_PORT:-27080}"
API_PORT="${E2E_API_PORT:-4112}"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/quotation-convert-e2e.XXXXXX")"
...
echo "Converting quotations"
if ! bun src/scripts/quotation-convert-e2e.ts; then
  echo
